...
static async Task<object?> CheckPluginUpdatesAsync(string server, string configDir, CancellationToken cancellationToken)
{
    ...
    http.DefaultRequestHeaders.UserAgent.ParseAdd("RustOpsRemoteAgent/1.0 (+https://umod.org)");
    ...
    foreach (var plugin in plugins)
    {
        ...
        var needsUpdate = !string.IsNullOrWhiteSpace(lookup.Latest) && !string.Equals(lookup.Latest, current, StringComparison.OrdinalIgnoreCase);
        var downloadUrl = needsUpdate ? lookup.DownloadUrl : null;
        updates.Add(new { plugin = plugin.Name, current, latest = lookup.Latest, downloadUrl, state = needsUpdate ? "update_available" : "current" });
    }
